...
export const themeComposable = () => {
  const colors = Object.entries(useAppColors()).map(([key, value]) => `'${key}': string`).join(',\n')
  return `import colors from '../theme/theme.colors.ts'

  type AppColors = {${colors}}

  export function useAppColors(): AppColors {
    return colors as AppColors
  }
`
}
