import { addPlugin, addTemplate, addTypeTemplate, useNuxt } from '@nuxt/kit'
...
export function optionProviderPluginGenerator(functionLike: `${string} => ${string}`) {
  ...
  const destPath = resolve(resolve(useNuxt().options.buildDir, 'autogenerated/client/plugins/provideThemeCode.ts'))
  createFile(destPath, `
    import { type ComputedRef, computed } from 'vue'
    import { type GlobalThemeOverrides } from 'naive-ui'

    export default defineNuxtPlugin((nuxtApp) => ({ provide: { themeCode: (appColors: Record<string, string>, themeName: ComputedRef<string>) => { ${themeFunction.getBodyText()} } } }))
  `)
